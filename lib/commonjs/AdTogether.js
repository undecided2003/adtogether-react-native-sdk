  ...
  static tryAutoDetectBundleId() {
    ...
    try {
      if (_reactNative.Platform.OS === 'android') {
        const constants = _reactNative.NativeModules.PlatformConstants;
        if (constants?.reactNativeVersion) {
          // On Android, the ServerHost module sometimes exposes the package name
          const appModule = _reactNative.NativeModules.RNDeviceInfo || _reactNative.NativeModules.AppInfo;
  ...
  static detectCountry() {
    ...
    try {
      if (_reactNative.Platform.OS === 'ios') {
        const settings = _reactNative.NativeModules.SettingsManager?.settings;
        ...
        const locale = _reactNative.NativeModules.I18nManager?.localeIdentifier;
        if (locale) {
          const parts = locale.replace('_', '-').split('-');
          if (parts.length >= 2) {
            const region = parts[parts.length - 1].toUpperCase();
